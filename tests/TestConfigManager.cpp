...
void TestConfigManager::testReadConfigFile_data()
{
    ...
    QTest::newRow("value with inline comment") << "key=value with # comment # inline comment\n" << settings5;
    ...
    QTest::newRow("value with inline comment and quotes") << "key=\"value with # comment\" # inline comment\n" << settings7;
...
void TestConfigManager::testReadConfigFile()
{
    ...
    for (auto it = expectedSettings.constBegin(); it != expectedSettings.constEnd(); ++it) {
...
void TestConfigManager::testSaveConfigFile_data()
{
    ...
    QTest::newRow("value with quotes") << settings5 << "key=value with \"quotes\"\n";
}
...
void TestConfigManager::testSaveConfigFile()
{
    ...
    for (auto it = settingsToSave.constBegin(); it != settingsToSave.constEnd(); ++it) {
        QVERIFY(actualSettings.contains(it.key()));
        QCOMPARE(actualSettings.value(it.key()), it.value());
    }
