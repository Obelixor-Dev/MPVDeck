...
InterfaceOsdViewModel::InterfaceOsdViewModel(QObject *parent)
    : QObject(parent) {}
...
void InterfaceOsdViewModel::setOsdDuration(int osdDuration) {
  if (osdDuration < MIN_OSD_DURATION) {
    qWarning() << "Invalid OSD duration:" << osdDuration;
    return;
  }
...
auto InterfaceOsdViewModel::screenshotFormat() const -> QString {
  return m_screenshotFormat;
}

void InterfaceOsdViewModel::setScreenshotFormat(
...
auto InterfaceOsdViewModel::screenshotDirectory() const -> QString {
...
auto InterfaceOsdViewModel::screenshotTemplate() const -> QString {
