...
Q_LOGGING_CATEGORY(lcAudioViewModel,
                   "mpvdeck.audioviewmodel") // New logging category
...
auto AudioViewModel::mute() const -> bool
{
  return m_mute;
}
...
auto AudioViewModel::volume() const -> int
{
  return m_volume;
}
...
auto AudioViewModel::audioDevice() const -> const QString&
{
...
void AudioViewModel::setAudioDevice(const QString& audioDevice)
{
  ...
  {
    qCWarning(lcAudioViewModel)
        << "Attempted to set invalid audio device:" << audioDevice;
    return;
  }
...
auto AudioViewModel::audioChannels() const -> const QString&
{
...
{
  if(!audioChannelLayouts().contains(audioChannels))
  {
    qCWarning(lcAudioViewModel)
        << "Attempted to set invalid audio channels:" << audioChannels;
    return;
  }
...
auto AudioViewModel::audioDevices() -> QStringList
{
  return {"auto", "alsa", "pulse", "jack", "oss"};
}
