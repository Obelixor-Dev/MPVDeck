...
class PerformanceCachingViewModel : public QObject
{
  ...
  public:
    ...
    [[nodiscard]] auto demuxerMaxBytes() const -> const QString&;
    void               setDemuxerMaxBytes(const QString& demuxerMaxBytes);

    [[nodiscard]] auto hwdec() const -> const QString&;
    void               setHwdec(const QString& hwdec);

    [[nodiscard]] static auto hwdecOptions() -> QStringList;

    [[nodiscard]] auto hwdecCodecs() const -> const QString&;
  ...
  private:
    bool m_cache     = true;
    int  m_cacheSecs = MPVDeck::Constants::
        DEFAULT_CACHE_SECS; // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes)
